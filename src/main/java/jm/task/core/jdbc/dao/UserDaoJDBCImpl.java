...
public class UserDaoJDBCImpl implements UserDao {
    ...
    public void saveUser(String name, String lastName, byte age) {
        try (PreparedStatement ptmt = connection.prepareStatement("INSERT INTO `new_schema`.`User` (name, lastName, age) values (?,?,?)")) {
    ...
    public void removeUserById(long id) {
        try (PreparedStatement ptmt = connection.prepareStatement("delete from `new_schema`.`User` where id = ?;")){
            ptmt.setLong(1, id);
            ptmt.executeUpdate();
        } catch (SQLException e) {
            System.out.println(e.getMessage());
